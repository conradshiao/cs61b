...
public class PossibleMoves {
    public static class Map extends Mapper<IntWritable, MovesWritable, IntWritable, IntWritable> {
         ...
         * 
         * Parents should be keys of output to organize the tree.
         */
        @Override
        public void map(IntWritable key, MovesWritable val, Context context) throws IOException, InterruptedException {
            /* YOU CODE HERE */
        	String currentBoard = Proj2Util.gameUnhasher((int) key.get(), boardWidth, boardHeight);
        	char turn = OTurn ? 'O' : 'X';
        	if (val.getStatus() == 0) {
        		for (int i = 0; i < boardWidth * boardHeight; i += boardHeight) {
        			for (int j = 0; j < boardHeight; j++) {
        				if (currentBoard.charAt(j + i) == ' ') {
        					StringBuilder temp = new StringBuilder(currentBoard);
        					temp.setCharAt(j + i, turn);
        					context.write(new IntWritable(
        							Proj2Util.gameHasher(temp.toString(), boardWidth, boardHeight)), key);
        					break;
        				}
        			}
        		}
    ...
    public static class Reduce extends Reducer<IntWritable, IntWritable, IntWritable, MovesWritable> {
        ...
        public void reduce(IntWritable key, Iterable<IntWritable> values, Context context) throws IOException, InterruptedException {
            ...
            int[] parents = new int[temp.size()];
            for (int i = 0; i < parents.length; i++) {
                parents[i] = (int) temp.get(i);
            }
            int status = 0;
            boolean finished = Proj2Util.gameFinished(Proj2Util.gameUnhasher(
                    key.get(), boardWidth, boardHeight), boardWidth, boardHeight, connectWin);
            if (finished) {
                status = OTurn ? 1 : 2;
            } else if (lastRound /*!finished given by else if*/) {
                status = 3;
            }
            context.write(key, new MovesWritable(status, 0, parents));
        }
    }
}
