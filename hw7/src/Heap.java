...
class Heap<T extends Comparable<? super T>> {
    ...
    void printLarger(T x) {
        printLargerHelper(x, 0);
    }

    /** Helper method for printLarger that prints all values that are >= X
     *  starting from INDEX. */
    private void printLargerHelper(T x, int index) {
        T item = data.get(index);
        if (item.compareTo(x) >= 0) {
            System.out.println(item);
            int left = 1 + 2 * index;
            if (left < data.size()) {
                printLargerHelper(x, left);
            }
            int right = 2 * index + 2;
            if (right < data.size()) {
                printLargerHelper(x, left);
