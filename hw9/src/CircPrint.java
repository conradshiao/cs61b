...
public class CircPrint {
    ...
    /** Helper method for write that returns if value V is circular. */
    static boolean checkCircular(Value v) {
        if (v.atomp()) {
            return false;
        } else {
            Value head = v.car();
    ...
    /** Helper method for write that returns the elements in each circular
     *  repetition of V, given that V is circular. */
    static int number(Value v) {
        if (v.atomp() || v.nullp()) {
            return 0;
        } else {
            Value initialHead = v.car();
            v = v.cdr();
            int count = 1;
            while (v.nullp()) {
                if (v.car() == initialHead) {
                    return count + 1;
                } else {
                    count++;
                }
