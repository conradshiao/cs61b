...
class Bitsort {
    ...
    private static void sortOnBit(int[] A, int p) {
        ...
        buckets.add(new ArrayList<Integer>());
        ArrayList<Integer> bucketOfOnes = buckets.get(1);
        ArrayList<Integer> bucketOfZeros = buckets.get(0);
        for (int integer : A) {
            if ((mask & integer) == 0) {
                bucketOfZeros.add(integer);
            } else {
                bucketOfOnes.add(integer);
            }
        }
        for (int i = 0; i < A.length; i++) {
            if (!bucketOfZeros.isEmpty()) {
                A[i] = bucketOfZeros.remove(0);
            } else {
                A[i] = bucketOfOnes.remove(0);
            }
        }
    }
